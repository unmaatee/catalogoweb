...
const state = {
    ...
    const modalMedia = imageModal.querySelector(".image-modal-media");
    ...
    function parseItemId(value) {
      // Los productos usan id numérico, pero el combo usa id de texto.
      // Esto evita que "combo-diario" se transforme en NaN y no pueda eliminarse del carrito.
    ...
    function renderModalThumbs(name) {
      ...
      modalProductThumbs.innerHTML = modalGallery.map((img, index) => `
        <button class="image-modal-thumb ${index === modalGalleryIndex ? "active" : ""}" type="button" data-modal-image-index="${index}" aria-label="Ver imagen ${index + 1} de ${name}">
          <img src="${img}" alt="${name} vista ${index + 1}" />
        </button>
      `).join("");

      modalProductThumbs.querySelectorAll("[data-modal-image-index]").forEach(button => {
        button.addEventListener("click", () => {
          setModalGalleryImage(Number(button.dataset.modalImageIndex), name);
        });
      });
    }

    function setModalGalleryImage(index, name) {
      if (!modalGallery.length) return;
      modalGalleryIndex = (index + modalGallery.length) % modalGallery.length;
      modalProductImage.src = modalGallery[modalGalleryIndex];
      modalProductImage.alt = `${name} vista ${modalGalleryIndex + 1}`;
      modalProductThumbs.querySelectorAll(".image-modal-thumb").forEach((item, idx) => {
        item.classList.toggle("active", idx === modalGalleryIndex);
      });
      const activeThumb = modalProductThumbs.querySelector(`.image-modal-thumb:nth-child(${modalGalleryIndex + 1})`);
      if (activeThumb) {
    ...
    function openImageModal(productId) {
      ...
      modalGallery = product.images && product.images.length ? product.images : [product.image || placeholderImage(product.name)];
      modalGalleryIndex = 0;
      ...
      renderModalThumbs(product.name);
      setModalGalleryImage(0, product.name);
    ...
    function openComboModal() {
      ...
      modalGallery = [FEATURED_COMBO.image];
      modalGalleryIndex = 0;
      ...
      renderModalThumbs(FEATURED_COMBO.name);
      setModalGalleryImage(0, FEATURED_COMBO.name);
      imageModal.classList.add("show");
      imageModal.setAttribute("aria-hidden", "false");
    }
    ...
    productGrid.addEventListener("click", event => {
      ...
      if (addButton) {
        addToCart(parseItemId(addButton.dataset.addId));
        return;
      }

      if (imageButton) {
        openImageModal(parseItemId(imageButton.dataset.imageId));
      }
    });
    ...
    cartItems.addEventListener("click", event => {
      ...
      if (removeButton) removeFromCart(parseItemId(removeButton.dataset.removeId));
      if (increaseButton) changeQty(parseItemId(increaseButton.dataset.increaseId), "increase");
      if (decreaseButton) changeQty(parseItemId(decreaseButton.dataset.decreaseId), "decrease");
    });
    ...
    modalMedia.addEventListener("touchend", event => {
      ...
      const galleryName = modalProductName.textContent || "Producto";
      setModalGalleryImage(modalGalleryIndex + direction, galleryName);
    }, { passive: true });

    document.addEventListener("keydown", event => {
      if (!imageModal.classList.contains("show") || modalGallery.length <= 1) return;
      if (event.key === "ArrowLeft") {
        setModalGalleryImage(modalGalleryIndex - 1, modalProductName.textContent || "Producto");
      }
